# 📌프로젝트 소개
* 카프카를 사용하여, DB 쓰기 작업 및 알림 발송을 비동기 처리
* k6를 사용하여 50명이 1초마다 API 요청하도록 과부하 테스트 
* 알림 발송은 실제로 하지 않고 Thread.sleep(1000) 처리
...
## 💻카프카 프로듀서 서버  
* **POST - http://localhost:8080/orders/async** 로 API요청 시 주문을 DB에 저장 후 알림 발송 메시지를 kafka에 전송
* 테스트 데이터:
  ```bash
  {
    "userId": 1,
...
* 메시지를 꺼내 Thread.sleep(1000)으로 알림 발송 후 주문 테이블의 알림 컬럼 update
